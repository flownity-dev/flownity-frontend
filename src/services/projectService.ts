...
import { mapToProjectStatus } from '../utils';
...
const transformApiProject = (apiProject: any): Project => {
	...
	const workflowStatus = mapToProjectStatus(apiProject.status);

	return {
		id: projectId.toString(),
		name: apiProject.project_title || apiProject.name || "Untitled Project",
		startDate: apiProject.start_date || apiProject.due_from ? new Date(apiProject.start_date || apiProject.due_from) : null,
		endDate: apiProject.end_date || apiProject.due_to ? new Date(apiProject.end_date || apiProject.due_to) : null,
		status: workflowStatus,
		deleted_at: apiProject.deleted_at,
	};
};
	...
	try {
		...
		if (Array.isArray(response.data)) {
			...
			return {
				...
				totalPages: 1,
				currentPage: 1,
				totalItems: transformedProjects.length,
			};
		}
		...
		if (response.data && typeof response.data === "object") {
			...
			if (
				...
				response.data.data &&
				"projects" in response.data.data &&
				Array.isArray(response.data.data.projects)
			) {
				const apiProjects = response.data.data.projects as ApiProject[];
			...
			if ("data" in response.data && Array.isArray(response.data.data)) {
				...
				return {
					...
					totalItems: response.data.totalItems || transformedProjects.length,
				};
			}
			...
			console.error(
				"Could not find projects array in response structure:",
				response.data
			);
			return {
				data: [],
...
const transformApiProjectToDetail = (
	apiProject: any
): import("../types/common.types").ProjectDetailData => {
	// Handle different possible ID field names
	const projectId = apiProject.id || apiProject.project_id;
	if (!projectId) {
		throw new Error("Project data missing ID field");
	...
	return {
		...
		status: workflowStatus,
		startDate: apiProject.due_from ? new Date(apiProject.due_from) : null,
		endDate: apiProject.due_to ? new Date(apiProject.due_to) : null,
...
export const getProjectById = async (
	id: string
): Promise<import("../types/common.types").ProjectDetailData> => {
	try {
...
export const getProjectFlow = async (
	projectId: string
